...
\begin{frame}
	...
	Nonostante si distingua Naive Bayes con feature selection e sensitive learning per
	il valore massimo più alto, la sua variabilità è molto elevata. In termini
	di compromesso tra valori di F1-score elevati e variabilità bassa, non risulta
	semplice scegliere un classificatore. Tra tutte le soluzioni proposte, IBk senza
	feature selection sembra essere la migliore (in particolare IBk con SMOTE).
	Osserviamo comunque, come l'applicazione di feature selection risulti in
	F1-score e variabilità bassa, senza accade esattamente il contrario.
...
\begin{frame}
	...
	Il kappa \textbf{peggiore} è sicuramente quello di Naive Bayes 
	senza feature selection con SMOTE e ancora no fe.sel sia con che 
	senza sens. learning.
	IBk si riconferma il migliore, in particolare con l'applicazione di feature 
	selection, no cost sensitivity (anche se usare sens. learning con IBk + feature
	selection non sembra essere una
	cattiva idea: il valore massimo è inferiore, ma non troppo, rispetto al precedente ma la variabilità più bassa, con un
...
\begin{frame}
	...
	valori minimi e mediane più bassi, oltre che elevata variabilità.
