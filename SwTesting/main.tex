...
\begin{document}
	...
	fatto che l'intervallo è $(\text{startTs},\text{endTs}]$ (startTs esclusivo, endTs inclusivo), ovvero, se in startTs passo il valore $t(e_1)$, allora
	...
	è contare il numero di eventi $e_k$ che rientrano nel range $0 \le t(e_k) \le \text{referenceTime}$.
	...
	referenceTime fuori dalla finestra ("a sinistra"), nel mezzo e poi fuori ("a destra").
	...
	Dato che sia i criteri di adeguatezza, che il mutation score dei metodi testati è praticamente quasi massima,
	...
	\ceqtable{getSlidingCountTimestamps}{WindowManager}{1}{\storm}{
			...
			\tcell{
				\ceq{$< 0$}\\
				\ceq{timestamp $\ge 0$ di evento che non è incluso nella finestra\\ di eventi attuale}\\
				\ceq{timestamp $\ge 0$ di evento che è incluso nella finestra\\ di eventi attuale}}
		\newtrow
			\tcell{endTs} &
	...
	\bvtable{getSlidingCountTimestamps}{WindowManager}{1}{\storm}{
		...
		\newtrow
			\tcell{startTs} &
			\tcell{\ceq{timestamp $\ge 0$ di evento che non è incluso nella finestra\\ di eventi attuale}} &
			\tcell{$0$}
		\newtrow
			\tcell{startTs} &
			\tcell{\ceq{timestamp $\ge 0$ di evento che è incluso nella finestra\\ di eventi attuale}} &
			\tcell{$t(e_1)$}
		\newtrow
			\tcell{endTs} &
			\tcell{\ceq{=startTs}} &
