...
\begin{document}
	...
	e test cases individuati. Nelle classi d'eq.: la \ceq{config invalida} è vuota (non ha impostato i parametri di configurazione 
	della finestra), il \ceq{context invalido} è l'oggetto TopologyContext i cui argomenti del
	...
	\textit{"Map\{(TOPOLOGY\_\dots,4)\}"} (chiavi di configurazione documentate 
	\href{https://storm.apache.org/releases/2.6.2/javadocs/org/apache/storm/Config.html}{qui}) 
	si intende la creazione di una Map<String,Object> e l'aggiunta di coppie chiave-valore con il metodo put
	...
	timestamp (e quindi tutte le tuple ricevute precedentemente, in senso temporale), per la configurazione del SUT, 
	lo sliding interval è di 1500ms, cosi come la window length. 
	Questo significa che, ad esempio, eventi con timestamp 10, 20, 500, 1400 ricadano nella prima finestra, 
	mentre altri eventi con timestamp 1600, 1800, 2100, 2900 nella seconda, 3000, 3100, nella terza, 
	e così via fino a 4500ms, \dots Come per la precedente iterazione (con la differenza dell'utilizzo del timestamp di evento, 
	e non del conteggio del num. di eventi), l'invocazione del metodo execute del bolt avviene ad ogni
	1500ms di differenza (nella finestra appena scaduta: se l'ultimo evento è compreso nella finestra di cui effettuare 
	sliding, viene incluso, altrimenti no, e ricade nella finestra successiva. 
	La finestra appena scaduta viene forwardata al bolt tramite invocazione a execute)
	Per il mocked bolt, per questa iterazione, è necessario simulare anche 
	\href{https://storm.apache.org/releases/2.6.2/javadocs/org/apache/storm/topology/IWindowedBolt.html#getTimestampExtractor()}
	{getTimestampExtractor} di 
	\href{https://storm.apache.org/releases/2.6.2/javadocs/org/apache/storm/topology/IWindowedBolt.html}
	{IWindowedBolt} per ritornare un 
	\href{https://storm.apache.org/releases/2.6.2/javadocs/org/apache/storm/topology/TupleFieldTimestampExtractor.html}
	{TupleFieldTimestampExtractor} 
	con lo stesso nome del campo dell'iterazione precedente, in modo da non dover cambiare eccessivamente l'implementazione.
	...
	L'integration test è stato effettuato con BookKeeper, in particolare la classe NetworkTopologyImpl, metodo 
	\href{https://bookkeeper.apache.org/docs/latest/api/javadoc/org/apache/bookkeeper/net/NetworkTopologyImpl.html#isOnSameRack(org.apache.bookkeeper.net.Node,org.apache.bookkeeper.net.Node)}
	{isOnSameRack}.
	In tabella \ref{\gettablelabel{ceq}{isOnSameRack}{NetworkTopologyImpl}{integration test}{\bookkeeper}}, 
	\ref{\gettablelabel{bv}{isOnSameRack}{NetworkTopologyImpl}{integration test}{\bookkeeper}} e
	\ref{\gettablelabel{testc}{isOnSameRack}{NetworkTopologyImpl}{integration test}{\bookkeeper}} sono presenti
	classi d'equivalenza, boundary values e test cases elaborati tramite approccio multidimensionale.
	I test falliti sono i primi 7, e rivelano una gestione errata degli argomenti: IllegalArgumentException non viene mai
	lanciata nonostante diversa indicazione sulla documentazione del metodo. 
	I test falliti vengono disabilitati commentandoli.
	La reliability stimata è 0.3, ma sempre secondo assunzione di profilo operazionale con distribuzione uniforme.
	In figura \ref{\getpicturelabel{bk/failsafe-summary-it}} è presente il resoconto di Maven Failsafe.
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			mutation$\;\rightarrow\;$integration-test$\;\rightarrow\;$report-packaging$\;\rightarrow\;$package-upload): 
	...
	\makepicture{17cm}{1cm}{bk/controlflow-NetworkTopologyImpl-2}
				{\classcfcovcaption{NetworkTopologyImpl}{2}{\bookkeeper}}
				
	\makepicture{17cm}{0.65cm}{bk/controlflow-add-NetworkTopologyImpl-2}
				{\methodcfcovcaption{add}{NetworkTopologyImpl}{2}{\bookkeeper}}
				
	\makepicture{17cm}{0.6cm}{bk/controlflow-getDistance-NetworkTopologyImpl-2}
				{\methodcfcovcaption{getDistance}{NetworkTopologyImpl}{2}{\bookkeeper}}
				
	\makepicture{17cm}{0.65cm}{bk/controlflow-pseudoSortByDistance-NetworkTopologyImpl-2}
	...
	\makepicture{13cm}{12cm}{bk/mutation-pseudoSortByDistance-NetworkTopologyImpl-2}
				{\mutmethodcaption{pseudoSortByDistance}{NetworkTopologyImpl}{2}{\bookkeeper}{13}{20}}
				
	\makepicture{6cm}{12cm}{bk/dataflow-add-NetworkTopologyImpl-2}
				{\alldfcovcaption{add}{NetworkTopologyImpl}{2}{\bookkeeper}}
				
	\makepicture{8cm}{20cm}{bk/dataflow-getDistance-1-NetworkTopologyImpl-2}
				{\alldfcovcaption{getDistance}{NetworkTopologyImpl}{2}{\bookkeeper}}
				
	\makepicture{8cm}{20cm}{bk/dataflow-getDistance-2-NetworkTopologyImpl-2}
				{\alldfcovcaption{getDistance}{NetworkTopologyImpl}{2}{\bookkeeper}}
				
	\makepicture{8cm}{20cm}{bk/dataflow-pseudoSortByDistance-1-NetworkTopologyImpl-2}
				{\alldfcovcaption{pseudoSortByDistance}{NetworkTopologyImpl}{2}{\bookkeeper}}
				
	\makepicture{8cm}{20cm}{bk/dataflow-pseudoSortByDistance-2-NetworkTopologyImpl-2}
				{\alldfcovcaption{pseudoSortByDistance}{NetworkTopologyImpl}{2}{\bookkeeper}}
	...
	\makepicture{17cm}{13cm}{bk/mutation-read-BufferedChannel-1}
	...
	\testctable{getEventCount}{WindowManager}{1}{\storm}{
			\tcell{$-1$} &
			\tcell{IllegalArgumentException viene lanciata} &
			\tcell{referenceTime < 0}
		\newtrow
			\tcell{$0$} &
			\tcell{Ritorno valore 0} &
			\tcell{Nessun evento rientra}
		\newtrow
			\tcell{$t(e_1)$} &
	...
	%
	% Iter: 2, Class: WindowManager, Project: Storm
	%
	...
	\makepicture{17cm}{0.75cm}{storm/controlflow-getEarliestEventTs-WindowManager-2}
				{\methodcfcovcaption{getEarliestEventTs}{WindowManager}{2}{\storm}}
				
	\makepicture{17cm}{0.65cm}{storm/controlflow-add-WindowManager-2}
				{\methodcfcovcaption{add}{WindowManager}{2}{\storm}}
				
	\makepicture{17cm}{0.70cm}{storm/controlflow-add-realoverload-WindowManager-2}
				{Overload: \methodcfcovcaption{add}{WindowManager}{2}{\storm}}
				
	\makepicture{17cm}{1.75cm}{storm/mutation-WindowManager-2}
				{\mutclasscaption{WindowManager}{2}{\storm}}
				
	\makepicture{15cm}{4cm}{storm/mutation-getEarliestEventTs-WindowManager-2}
				{\mutmethodcaption{getEarliestEventTs}{WindowManager}{2}{\storm}{5}{5}}
	
	\makepicture{15cm}{8cm}{storm/mutation-add-WindowManager-2}
				{\mutmethodcaption{add}{WindowManager}{2}{\storm}{2}{3}}
				
	\makepicture{13cm}{8cm}{storm/dataflow-getEarliestEventTs-WindowManager-2}
				{\alldfcovcaption{getEarliestEventTs}{WindowManager}{2}{\storm}}
				
	\makepicture{12cm}{6cm}{storm/dataflow-add-WindowManager-2}
				{\alldfcovcaption{add}{WindowManager}{2}{\storm}}
				
	\makepicture{17cm}{1.25cm}{storm/controlflow-missingmethods-WindowManager-2}
				{\methodcfcovcaption{scanEvents e onTrigger}{WindowManager}{2}{\storm}}
	...
	\reltable{WindowManager}{\storm}{
		...
		\newtrow
			...
			\tcell{$0$} &
			\tcell{
				tabella $\ref{\gettablelabel{testc}{add}{WindowManager}{2}{\storm}}$} &
			\tcell{\unifdist{0.11}} &
			\tcell{$1$}
		\newtrow
	}
	...
	%
	% Iter: 1, Class: WindowedBoltExecutor, Project: Storm
	%
	...
	\testctable{prepare}{WindowedBoltExecutor}{1}{\storm}{
		...
		\newtrow
			...
			\tcell{Ci si può aspettare una\\ unchecked exception, non viene\\ specificata nella documentazione}
		\newtrow
			\tcell{null,\\invalidTopologyContext(),\\validOutputCollector()} &
			\tcell{RuntimeException viene lanciata} &
			\tcell{Ci si può aspettare una\\ unchecked exception, non viene\\ specificata nella documentazione}
		\newtrow
			\tcell{null,\\invalidTopologyContext(),\\invalidOutputCollector()} &
			\tcell{RuntimeException viene lanciata} &
			\tcell{Ci si può aspettare una\\ unchecked exception, non viene\\ specificata nella documentazione}
		\newtrow
			\tcell{invalidConfig(),\\null,\\null} &
		...
		\newtrow
			...
			\tcell{Ci si può aspettare una\\ unchecked exception, non viene\\ specificata nella documentazione}
		\newtrow
			\tcell{invalidConfig(),\\makeTopologyContext(),\\null} &
			\tcell{RuntimeException viene lanciata} &
			\tcell{Ci si può aspettare una\\ unchecked exception, non viene\\ specificata nella documentazione}
		\newtrow
			\tcell{invalidConfig(),\\makeTopologyContext(),\\validOutputCollector()} &
			\tcell{IllegalArgumentException viene lanciata} &
			\tcell{Nessuno degli argomenti è null\\ e vengono esplicitamente indicati\\param invalidi}
		\newtrow
			\tcell{invalidConfig(),\\makeTopologyContext(),\\invalidOutputCollector()} &
			\tcell{IllegalArgumentException viene lanciata} &
			\tcell{Nessuno degli argomenti è null\\ e vengono esplicitamente indicati\\param invalidi}
		\newtrow
			\tcell{invalidConfig(),\\invalidTopologyContext(),\\null} &
			\tcell{RuntimeException viene lanciata} &
			\tcell{Ci si può aspettare una\\ unchecked exception, non viene\\ specificata nella documentazione}
		\newtrow
			\tcell{invalidConfig(),\\invalidTopologyContext(),\\validOutputCollector()} &
			\tcell{IllegalArgumentException viene lanciata} &
			\tcell{Nessuno degli argomenti è null\\ e vengono esplicitamente indicati\\param invalidi}
		\newtrow
			\tcell{invalidConfig(),\\invalidTopologyContext(),\\invalidOutputCollector()} &
			\tcell{IllegalArgumentException viene lanciata} &
			\tcell{Nessuno degli argomenti è null\\ e vengono esplicitamente indicati\\param invalidi}
		\newtrow
			\tcell{validConfig(),\\null,\\null} &
		...
		\newtrow
			...
			\tcell{Ci si può aspettare una\\ unchecked exception, non viene\\ specificata nella documentazione}
		\newtrow
			\tcell{validConfig(),\\makeTopologyContext(),\\null} &
			\tcell{RuntimeException viene lanciata} &
			\tcell{Ci si può aspettare una\\ unchecked exception, non viene\\ specificata nella documentazione}
		\newtrow
			\tcell{validConfig(),\\makeTopologyContext(),\\validOutputCollector()} &
			\tcell{Terminazione con successo} &
			\tcell{Il metodo non solleva eccezioni,\\il metodo prepare() del bolt\\viene invocato}
		\newtrow
			\tcell{validConfig(),\\makeTopologyContext()\\invalidOutputCollector()} &
			\tcell{IllegalArgumentException viene lanciata} &
			\tcell{Nessuno degli argomenti è null\\ e vengono esplicitamente indicati\\param invalidi}
		\newtrow
			\tcell{validConfig(),\\invalidTopologyContext(),\\null} &
			\tcell{RuntimeException viene lanciata} &
			\tcell{Ci si può aspettare una\\ unchecked exception, non viene\\ specificata nella documentazione}
		\newtrow
			\tcell{validConfig(),\\invalidTopologyContext(),\\validOutputCollector()} &
			\tcell{IllegalArgumentException viene lanciata} &
			\tcell{Nessuno degli argomenti è null\\ e vengono esplicitamente indicati\\param invalidi}
		\newtrow
			\tcell{validConfig(),\\invalidTopologyContext()\\invalidOutputCollector()} &
			\tcell{IllegalArgumentException viene lanciata} &
			\tcell{Nessuno degli argomenti è null\\ e vengono esplicitamente indicati\\param invalidi}
		\newtrow
	}
	
	\ceqtable{execute}{WindowedBoltExecutor}{1}{\storm}{
			\tcell{input} &
			\tcell{\ceq{null}\\\ceq{tupla}}
		\newtrow
	}
	
	\bvtable{execute}{WindowedBoltExecutor}{1}{\storm}{
			\tcell{input} &
			\tcell{\ceq{null}} &
			\tcell{null}
		\newtrow
			\tcell{input} &
			\tcell{\ceq{tupla}} &
			\tcell{new TupleImpl(\\\;new GeneralTopologyContext(\dots, fields),\\\;values, \dots)}
		\newtrow
	}
	...
	\ceqtable{isOnSameRack}{NetworkTopologyImpl}{integration test}{\bookkeeper} {
			\tcell{node1} &
			\tcell{\ceq{null}\\\ceq{nodo non in rete}\\\ceq{nodo in rete}} 
		\newtrow
			\tcell{node2} &
			\tcell{\ceq{null}\\\ceq{nodo non in rete}\\\ceq{nodo in rete}}
		\newtrow
	...
	\bvtable{isOnSameRack}{NetworkTopologyImpl}{integration test}{\bookkeeper} {
			\tcell{node1} &
			\tcell{\ceq{null}} &
			\tcell{null}
		\newtrow
			\tcell{node1} &
			\tcell{\ceq{nodo non in rete}} &
			\tcell{Node(/europe/it/lazio/roma/rack-1/bookie-14")}
		\newtrow
			\tcell{node1} &
			\tcell{\ceq{nodo in rete}} &
			\tcell{Node(/europe/it/lazio/roma/rack-1/bookie-1")}
		\newtrow
			\tcell{node2} &
			\tcell{\ceq{null}} &
	...
	\testctable{isOnSameRack}{NetworkTopologyImpl}{integration test}{\bookkeeper} {
			\tcell{null,null} &
			\tcell{IllegalArgumentException viene lanciata} &
			\tcell{Entrambi gli argomenti sono null}
		\newtrow
			\tcell{null, Node(/europe/\\it/lazio/roma/rack-1\\/bookie-1")} &
			\tcell{IllegalArgumentException viene lanciata} &
			\tcell{Primo argomento è null}
		\newtrow
			\tcell{Node(/europe/\\it/lazio/roma/rack-1\\/bookie-1"), null} &
			\tcell{IllegalArgumentException viene lanciata} &
			\tcell{Secondo argomento è null}
		\newtrow
			\tcell{Node(/europe/\\it/lazio/roma/rack-1/\\bookie-14"),\\ Node(/europe/\\it/lazio/roma/rack-1\\/bookie-1")} &
			\tcell{IllegalArgumentException viene lanciata} &
			\tcell{Il primo nodo non esiste nel cluster}
		\newtrow
			\tcell{Node(/europe/\\it/lazio/roma/rack-1/\\bookie-1"),\\ Node(/europe/\\it/lazio/roma/rack-1\\/bookie-14")} &
			\tcell{IllegalArgumentException viene lanciata} &
			\tcell{Il secondo nodo non esiste nel cluster}
		\newtrow
			\tcell{null, Node(/europe/\\it/lazio/roma/rack-1\\/bookie-14")} &
			\tcell{IllegalArgumentException viene lanciata} &
			\tcell{Primo arg null e secondo nodo\\ non esistente}
		\newtrow
			\tcell{Node(/europe/\\it/lazio/roma/rack-1\\/bookie-14"), null} &
			\tcell{IllegalArgumentException viene lanciata} &
			\tcell{Secondo argomento è null}
		\newtrow
			\tcell{Node(/europe/\\it/lazio/roma/rack-1\\/bookie-14"),\\ Node(/europe/\\it/lazio/roma/rack-1\\/bookie-14")} &
			\tcell{IllegalArgumentException viene lanciata} &
			\tcell{Primo e secondo nodo non\\ esistono nel cluster}
		\newtrow
			\tcell{Node(/europe/\\it/lazio/roma/rack-1\\/bookie-1"),\\ Node(/europe/\\it/lazio/roma/rack-1\\/bookie-2")} &
			\tcell{Ritorno valore true} &
			\tcell{Nodi esistenti nel cluster\\ e sullo stesso rack (rack-1)}
		\newtrow
			\tcell{Node(/europe/\\it/lazio/roma/rack-1\\/bookie-2"),\\ Node(/europe/\\it/lazio/roma/rack-2\\/bookie-3")} &
			\tcell{Ritorno valore false} &
			\tcell{Nodi esistenti nel cluster\\ ma su due rack diversi}
		\newtrow
	}
	
	\makepicture{16cm}{3.5cm}{bk/failsafe-summary-it}
		{Failsafe summary dell'integration test}
