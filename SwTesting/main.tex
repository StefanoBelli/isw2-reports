...
\begin{document}
	...
	\textbf{Per il metodo getDistance}
	
	GETDISTANCE: AGGIUNGI CASI DI TEST NULL (COMBINAZIONI) E NODI APPARTENENTI/NON APPARTENENTI (DOCS)
	PSEUDOSORTBYDISTANCE: STESSA COSA, IN PIU SPECIFICA CHE IN ENTRAMBI I CASI I TEST GENERATI RIGUARDANO
	I BOOKIES, DATA LA NATURA DEI METODI E DI BK
	...
	SPIEGA POPULATEDNETWORKTOPOLOGY
	SPIEGA GETNODE e BUILDNODE (se uso getnode su nodo non esistente... null)
	SPIEGA DISTANZA
	SPECIFICA CHE CI INTERESSA LA DISTANZA TRA I BOOKIES
	recupera documentazione getdistance per algoritmo distanza
	SPIEGA PERCHE HO USATO QUELL INTERFACCIA BUILDER
	DESCRIVI FALLIMENTI (specifica dist unif. ma in realtà...)
	APPROCCIO MULTIDIM
	CONSIDERAZIONI RELIABILITY: RISULTATI ESATTI SU NODI PRESENTI
	...
	RICORDA: APPROCCIO sort MULTIDIMENSIONALE, RICORDA CHE NODO NON PRESENTE IN RETE NON è UTILE: BASSA PROB
	DI UTILIZZO
	...
	\ceqtable{pseudoSortByDistance}{NetworkTopologyImpl}{2}{\bookkeeper}{
		...
		\newtrow
			\tcell{nodes} &
			\tcell{\ceq{null}\\\ceq{array con tutti i nodi presenti in rete}}
		\newtrow
	}
	...
	\bvtable{pseudoSortByDistance}{NetworkTopologyImpl}{2}{\bookkeeper}{
		...
		\newtrow
			\tcell{nodes} & \tcell{\ceq{array con tutti i nodi\\ presenti in rete}} & 
			\tcell{[Node("/europe/it/lazio/roma/rack-1/bookie-1"),\\
